...
sys.path.append("/media/mary/T7/code/LE_PGD/ablation/SADAJEM")

from SADAJEM.ExpUtils import *
from models.jem_models_old import F, CCF
from SADAJEM.utils import plot
from SADAJEM.Task.eval_buffer import cond_is_fid
...
def init_from_centers(arg):
    ...
    centers = t.load("/media/mary/T7/code/LE_PGD/ablation/data/cifar10_mean.pt")
    covs = t.load("/media/mary/T7/code/LE_PGD/ablation/data/cifar10_cov.pt")
    ...
    for i in range(arg.n_classes):
        ...
        dist = MultivariateNormal(
            mean,
            covariance_matrix=cov + 1e-4 * t.eye(int(np.prod(size))).to(arg.device),
        )
        buffer.append(
            dist.sample((bs // arg.n_classes,)).view([bs // arg.n_classes] + size).cpu()
        )
...
def init_pca(f, arg, dataset, y, device):
    init_images, _ = utils.get_init_images(
        replay_buffer=[],
        batch_size=32,
        y=y,
        inizialization="pca",
...
def pca_samples(f, arg, dataset, device):
    import enumeration
    from PIL import Image
    ret_samples = []
    # for batch in range(arg.batch_size // 32 + 1):
    classes = enumeration.dataset_num_classes[dataset]
    
    for _ in range(arg.n_sample_steps):
        for cl in range(3):
            init_imgs, samples, ret_energy = utils.sample_q_SGLD(
                model=f,
                replay_buffer=[],
                step_size=0.8,
            ...
            # get images and save
            imgs = samples.clone().detach().cpu().numpy()
            for i, im in enumerate(imgs):
                
                im = np.transpose(im, (1, 2, 0)) * 255
                im = np.uint8(im)  # Convert to uint8 if not already
                pil_img = Image.fromarray(im)
                
                pil_img.save(
                    f"/media/mary/T7/code/LE_PGD/ablation/imgs/test_sadajem/{cl}_{i}.png"
                )
                ret_samples.append(samples[i])
    
    # stack all images
    ret_samples = t.stack(ret_samples)
    print(ret_samples.shape)
...
def best_samples(f, replay_buffer, arg, device, fresh=False):
    ...
    for c in range(10):
        ...
        print(
            "P(x) min %.3f max %.3f"
            % (
                -each_metric[index_list].max().item(),
                -each_metric[index_list].min().item(),
            )
        )
        print(
            "Prob(y|x) max %.3f min %.3f"
            % (each_probs[index_list].max().item(), each_probs[index_list].min().item())
        )
    ...
    for i in range(arg.n_sample_steps):
        ...
        if (i + 1) % arg.print_every == 0 and save:
            ...
            if arg.print_every < 50:
                continue
            inc_score, std, fid = cond_is_fid(
                f, replay_buffer, arg, device, ratio=100000
            )
...
def logp_hist(f, arg, device):
    ...
    datasets = {
        ...
        "svhn": tv.datasets.SVHN(
            root="./data", transform=transform_test, download=True, split="test"
        ),
        "cifar100": tv.datasets.CIFAR100(
            root="./data", transform=transform_test, download=True, train=False
        ),
        "celeba": tv.datasets.CelebA(
            root="./data",
            download=True,
            split="test",
    ...
    for i, (name, scores) in enumerate(score_dict.items()):
        plt.hist(scores, label=name, bins=100, alpha=0.5, color=colors[i])
    plt.legend(loc="upper left")
    plt.xticks([])
    plt.yticks([])
    plt.savefig(
        arg.save_dir + "/jem_%s_logp.pdf" % arg.datasets[1],
        bbox_inches="tight",
        pad_inches=0.0,
    )
...
def OODAUC(f, arg, device):
    ...
    num_workers = 0 if arg.debug else 4
    dset_real = tv.datasets.CIFAR10(
        root="./data", transform=transform_test, download=True, train=False
    )
    dload_real = DataLoader(
        dset_real,
        batch_size=100,
        shuffle=False,
        num_workers=num_workers,
    ...
    print(
        "OOD scores %f of %s between cifar10 and %s using %s"
        % (score, arg.score_fn, args.ood_dataset, args.load_path)
    )
...
def test_clf(f, arg, device):
    ...
    if arg.dataset == "cifar_train":
        ...
        dset = tv.datasets.SVHN(
            root="./data", transform=transform_test, download=True, split="train"
        )
    ...
    num_workers = 0 if arg.debug else 4
    dload = DataLoader(
        dset, batch_size=100, shuffle=False, num_workers=num_workers, drop_last=False
    )
    ...
    for x_p_d, y_p_d in tqdm(dload):
        ...
        loss = (
            nn.CrossEntropyLoss(reduction="none")(logits, y_p_d).cpu().detach().numpy()
        )
        losses.extend(loss)
...
def main(arg):
    ...
    if arg.eval == "fid" and replay_buffer is not None:
        ...
        # inc_score, std, fid = cond_is_fid(
        #     f, replay_buffer, arg, device, ratio=0.1, eval="is"
        # )
    ...
    if arg.eval == "pca":
        pca_samples(f, arg, "cifar10" if "cifar" in args.dataset else "shvn", device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="batch",
        choices=[None, "none", "norm", "batch", "instance", "layer", "act"],
    )
