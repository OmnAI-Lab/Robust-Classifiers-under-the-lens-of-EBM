...
import sys
from dataset_loader import load_svhn, load_tiny_imagenet 
from art.attacks.evasion.carlini import (
    CarliniLInfMethod,
)
from art.estimators.classification import PyTorchClassifier

sys.path.append("../robustness/robustbenchmaster/robustbench")
sys.path.append(
    "../robustness/robustbenchmaster/robustbench/data.py"
)
sys.path.append(
    "../robustness/robustbenchmaster/robustbench/utils.py"
)

from robustbenchmaster.robustbench.data import (
    CORRUPTIONS_DICT,
    get_preprocessing,
    load_clean_dataset,
...
from robustbenchmaster.robustbench.model_zoo.enums import BenchmarkDataset, ThreatModel
...
def benchmark(
    ...
    if dataset_ == BenchmarkDataset.svhn:
        clean_x_test, clean_y_test = load_svhn(
            n_examples, data_dir, transforms_test=prepr
        )
    elif dataset_ == BenchmarkDataset.tiny_imagenet:
        clean_x_test, clean_y_test = load_tiny_imagenet(
            n_examples, data_dir, transforms_test=prepr
        )
    else:
...
def cw_benchmark(
    ...
    n_examples: int = 10000,
    dataset: Union[str, BenchmarkDataset] = BenchmarkDataset.cifar_10,
    threat_model: Union[str, ThreatModel] = ThreatModel.L2,
    to_disk: bool = False,
    model_name: Optional[str] = None,
    data_dir: str = "./data",
    device: Optional[Union[torch.device, Sequence[torch.device]]] = None,
    ...
    x_adv = attack.generate(clean_x_test.numpy())
    ...
    for corruption in tqdm(corruptions):
        for severity in range(1, 6):
            x_corrupt, y_corrupt = CORRUPTION_DATASET_LOADERS[dataset][threat_model](
                n_examples, severity, data_dir, shuffle=False, corruptions=[corruption]
            )  # ,
            # prepr=prepr)
